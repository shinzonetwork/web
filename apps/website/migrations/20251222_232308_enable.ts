...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "subtitle", "featured_image_id", "excerpt", "content", "meta_title", "meta_image_id", "meta_description", "generate_slug", "slug", "published_at", "updated_at", "created_at", "_status") SELECT "id", "title", "subtitle", "featured_image_id", "excerpt", "content", "meta_title", "meta_image_id", "meta_description", "generate_slug", "slug", "published_at", "updated_at", "created_at", "_status" FROM \`posts\`;`)
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  `)
  await db.run(sql`INSERT INTO \`__new_posts\`("id", "title", "subtitle", "featured_image_id", "excerpt", "content", "meta_title", "meta_image_id", "meta_description", "generate_slug", "slug", "published_at", "updated_at", "created_at") SELECT "id", "title", "subtitle", "featured_image_id", "excerpt", "content", "meta_title", "meta_image_id", "meta_description", "generate_slug", "slug", "published_at", "updated_at", "created_at" FROM \`posts\`;`)
  await db.run(sql`DROP TABLE \`posts\`;`)
  await db.run(sql`ALTER TABLE \`__new_posts\` RENAME TO \`posts\`;`)
  await db.run(sql`PRAGMA foreign_keys=ON;`)
  await db.run(sql`CREATE INDEX \`posts_featured_image_idx\` ON \`posts\` (\`featured_image_id\`);`)
