...
- `RECAPTCHA_SECRET_KEY`: Secret key for server-side reCAPTCHA verification. Create one in the [Google reCAPTCHA admin console](https://www.google.com/recaptcha/admin/create).
- `FAUCET_PRIVATE_KEY`: Private key for the funded ShinzoHub account that sends `$SHN`. Use a devnet faucet account only; never use a production or personally controlled key here.
