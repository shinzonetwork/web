...
`)
const EthereumBlockTransactionsByHashQuery = graphql(`
...
export const useEthereumBlockTransactions = (options: UseEthereumBlockTransactionsOptions) => {
  ...
  return useQuery({
    queryKey: ['ethereum', 'block-transactions', blockNumber ?? blockHash, blockNumber !== undefined ? 'blockNumber' : 'blockHash', offset, limit],
    enabled: blockNumber !== undefined || !!blockHash,
    staleTime: 1000 * 60,
    queryFn: async () => {
      let res;
      if (blockNumber !== undefined) {
        res = await execute(EthereumBlockTransactionsQuery, { offset, limit, blockNumber });
      } else {
        if (blockHash === undefined) {
          throw new Error(
            'useEthereumBlockTransactions: blockHash is required when blockNumber is omitted',
          );
        }
        res = await execute(EthereumBlockTransactionsByHashQuery, { offset, limit, hash: blockHash });
      }
