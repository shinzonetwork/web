import { getAssertion } from "@shinzo/shinzohub";
...
export async function GET(req: NextRequest) {
  ...
  try {
    ...
    const result = await getAssertion(client, { address, cosmosRestUrl });

    return Response.json(serializeAssertions(result));
  } catch {
    return Response.json({ error: "Failed to load assertion" }, { status: 500 });
  }
}
