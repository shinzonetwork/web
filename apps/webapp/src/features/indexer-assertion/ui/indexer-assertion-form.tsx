    ...
    const [isSubmitting, setIsSubmitting] = useState(false);
      ...
      const handleSubmit = async () => {
        ...
        if (!privateKey || !rpcEndpoint) {
          toast.error("Missing assertion config: RPC endpoint or private key.", TOAST_CONFIG);
          return;
        }
        ...
        try {
          ...
          const result = await adminIndexerAssertion({
            ...
            delegateDigest: new TextEncoder().encode(assertionFormData.delegateDigest),
            delegateSignature: fromHex(assertionFormData.delegateSignature.replace(/^0x/, "")),
          });
          ...
          toast.error(`Assertion failed (${result.code}): ${result.log}`, TOAST_CONFIG);
        } catch (error) {
